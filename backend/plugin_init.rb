...
# we are deduping repeat occurences of each $type_id
...
ArchivesSpaceService.loaded_hook do
  DB.open do |db|
    ...
    $dupes.each do |type_id, dupe_event_relationships|
      if dupe_event_relationships.any?
        Log.info "DELETING DUPLICATE #{type_id.to_s} EVENT RECORDS: #{Time.now}"
        count = 0
        ...
        Log.info "COMPLETED DELETING #{type_id.to_s} DUPLICATE EVENT RECORDS: [#{count.to_s}] #{Time.now}"
      end
    end
  end
end
